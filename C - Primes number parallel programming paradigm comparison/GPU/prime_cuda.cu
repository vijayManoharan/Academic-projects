...
__global__ void cudaBucketSort(float *array, float *pivots) {
    /* 1.call merge sort for each threads
     * 2.Range is given based on the pivot values.
     * 3.Each thread will sort its own data set.
     */
    int start = pivots[(blockIdx.x * blockDim.x) + threadIdx.x];
    int end = pivots[(blockIdx.x * blockDim.x) + threadIdx.x+1]-1;
    printf("pivot index %d\t start %d \t end %d\n", (blockIdx.x * blockDim.x) + threadIdx.x, start, end);
...
int main(int argc, char** argv) {
    ...
    int Range=10;
    int TotalThread = size/Range;
    int ThreadSize = TotalThread/BlockNum;
    
    //variables for pivots 
    int pivots[TotalThread + 1];
    float pivots_value[TotalThread + 1];
    ...
    float *dev_array, *dev_pivots;

  
    cudaMalloc((void **) &dev_array, size * sizeof (float));
    cudaMalloc((void **) &dev_pivots, (TotalThread + 1) * sizeof (float));
    ...
    cudaMemcpy(dev_pivots, pivots, (TotalThread + 1) * sizeof (float), cudaMemcpyHostToDevice);
