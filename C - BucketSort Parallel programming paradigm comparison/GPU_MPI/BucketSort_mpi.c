...
int main(int argc, char** argv) {
	...
	pivots_value[p_id+1] = quickSelect(array, 0, size - 1, (pivots[p_id]));
    pivots_value[p_id+1] = quickSelect(array, pivots[p_id], size - 1, (pivots[p_id+1]));
    
	printf("is the %d largest value %f\n", pivots[p_id+1], pivots_value[p_id+1]);
	
	/*for(i=0;i<size;i++)
			if(p_id==0)printf("%d-->%f\n",i,array[i]);
    */
    printf("\nmergesort starts \n");
	...
	for(i=subarray_size*p_id;i<subarray_size*(p_id+1);i++)
			{subarray[i-(subarray_size*p_id)]=array[i];
			
			}
	cuda_main(subarray,subarray_size,p_id);
		for(i=subarray_size*p_id;i<subarray_size*(p_id+1);i++)
			array[i]=subarray[i-pivots[p_id]];
